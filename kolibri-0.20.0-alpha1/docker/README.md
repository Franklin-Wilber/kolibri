...
### Local Source With Compose

From the repository root, build the image from this checkout and start Kolibri:
...
The Compose setup stores Kolibri data in the `kolibri-data` named volume. The
PostgreSQL service is kept behind the optional `postgres` profile because the
default local review setup uses Kolibri's SQLite storage.
